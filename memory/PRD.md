...
- `creator_profiles` (1:1 users where role=creator) — user_id(unique), name, instagram_handle?, instagram_profile_url?, profile_image_url?, email?, city?, address (the neighbourhood a brand filters on), full_address? (where post actually goes), niches[] (what they cover for a brand: cafe, brunch), genres[] (what they make: food, travel — a brand searches on one and briefs on the other), platforms[] (instagram|youtube, a closed list so the directory stays searchable), base_rate?, follower_count? (self-reported), verification_status(pending|verified|rejected), pending_review, payout_upi?, payout_account_name?, pan?, gstin?. niches and genres are stored lower-cased, since campaign suggestions match on them. Indexes: `user_id` unique, `verification_status`, `niches`.
